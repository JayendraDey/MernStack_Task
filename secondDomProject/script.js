...
const second_page_right_bottom = document.querySelector(
  ".second_page_right_bottom"
);
...
inp.forEach((input) => {
  input.addEventListener("input", (e) => {
    ...
    objForInp = { ...objForInp, [name]: value };
    console.log(objForInp);
  });
});
...
function isBokedService(booked_item_array) {
  ...
  if (booked_item_array.length === 0) {
    ...
    book_now_button.addEventListener("click", () => {
      ...
      document
        .querySelectorAll(".left_add_service_btn_Remove")
        .forEach((btn) => {
...
const select_service_card = (item) => {
  ...
  left_add_service_btn.addEventListener("click", () => {
    ...
    if (!filtered) {
      ...
      booked_item_array = booked_item_array.filter(
